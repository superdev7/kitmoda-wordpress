...
function wp_count_posts( $type = 'post', $perm = '' ) {
	...
	if ( false !== $counts ) {
		/** This filter is documented in kt-encased/post.php */
...
function wp_publish_post( $post ) {
	...
	/** This action is documented in kt-encased/post.php */
...
function wp_delete_attachment( $post_id, $force_delete = false ) {
	...
	if ( ! empty($meta['thumb']) ) {
		...
		if (! $wpdb->get_row( $wpdb->prepare( "SELECT meta_id FROM $wpdb->postmeta WHERE meta_key = '_wp_attachment_metadata' AND meta_value LIKE %s AND post_id <> %d", '%' . $wpdb->esc_like( $meta['thumb'] ) . '%', $post_id)) ) {
			...
			/** This filter is documented in kt-encased/functions.php */
...
function wp_get_attachment_url( $post_id = 0 ) {
	...
	if ( $file = get_post_meta( $post->ID, '_wp_attached_file', true) ) {
		...
		if ( ($uploads = wp_upload_dir()) && false === $uploads['error'] ) {
			...
			} elseif ( false !== strpos($file, 'ktmaterial/uploads') ) {
				$url = $uploads['baseurl'] . substr( $file, strpos($file, 'ktmaterial/uploads') + 18 );
			} else {
	...
	if ( is_ssl() && ! is_admin() && 'kt-usergate.php' !== $GLOBALS['pagenow'] ) {
