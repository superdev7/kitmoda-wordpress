 ...
 * @since 3.3.0
 *
 * Private, not included by default. See wp_editor() in kt-encased/general-template.php.
 */
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			...
			if ( $set['media_buttons'] ) {
				...
				if ( ! function_exists( 'media_buttons' ) )
					include( ABSPATH . 'magento-help/includes/media.php' );
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * 'myplugin' => 'http://mysite.com/ktmaterial/plugins/myfolder/mce_plugin.js'.
					 *
				...
				$mce_css = array(
					$dashicons,
					self::$baseurl . '/skins/wordpress/ktmaterial.css?' . $version
				);
