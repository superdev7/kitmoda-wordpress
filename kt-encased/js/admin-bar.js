...
if ( typeof(jQuery) != 'undefined' ) {
	if ( typeof(jQuery.fn.hoverIntent) == 'undefined' ) {
		/* jshint ignore:start */
		// hoverIntent v1.8.1 - Copy of kt-encased/js/hoverIntent.min.js
	...
	jQuery(document).ready(function($){
		...
		touchOpen = function(unbind) {
			adminbar.find('li.menupop').on('click.wp-mobile-hover', function(e) {
				...
				if ( el.parent().is('#magento-help-bar-root-default') && !el.hasClass('hover') ) {
					e.preventDefault();
		...
		$('#magento-help-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#magento-help-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});
		...
		adminbar.click( function(e) {
			if ( e.target.id != 'wpadminbar' && e.target.id != 'magento-help-bar-top-secondary' ) {
				return;
			}
		...
		if ( 'sessionStorage' in window ) {
			$('#magento-help-bar-logout a').click( function() {
				try {
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'magento-help-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			if ( t.id != 'wpadminbar' && t.id != 'magento-help-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			...
			if ( d.body && aB ) {
				...
				addEvent( document.getElementById('magento-help-bar-logout'), 'click', function() {
					if ( 'sessionStorage' in window ) {
						try {
							for ( var key in sessionStorage ) {
								if ( key.indexOf('wp-autosave-') != -1 )
