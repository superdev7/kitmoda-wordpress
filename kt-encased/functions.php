...
function do_robots() {
	...
	if ( '0' == $public ) {
		...
		$output .= "Disallow: $path/magento-help/\n";
		$output .= "Allow: $path/magento-help/admin-ajax.php\n";
	}
...
function wp_upload_dir( $time = null ) {
	...
	if ( empty( $upload_path ) || 'ktmaterial/uploads' == $upload_path ) {
	...
	if ( !$url = get_option( 'upload_url_path' ) ) {
		if ( empty($upload_path) || ( 'ktmaterial/uploads' == $upload_path ) || ( $upload_path == $dir ) )
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had ktmaterial/uploads for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as ktmaterial/uploads is used
			 *    there, and
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	/** This filter is documented in magento-help/includes/file.php */
 ...
 *
 * If a file exists in the ktmaterial directory named db-error.php, then it will
 ...
 *
 * Will remove magento-help links to retrieve only return URLs not in the magento-help
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'magento-help' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'kt-usergate.php' ) !== false ) {
			$path = preg_replace( '#/(magento-help/.*|kt-usergate.php)#i', '', $_SERVER['REQUEST_URI'] );
...
/**
 * Output the HTML that shows the kt-usergate dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
