...
function wp_set_auth_cookie( $user_id, $remember = false, $secure = '', $token = '' ) {
	if ( $remember ) {
		...
		/** This filter is documented in kt-encased/pluggable.php */
...
function auth_redirect() {
	...
	if ( $secure && !is_ssl() && false !== strpos($_SERVER['REQUEST_URI'], 'magento-help') ) {
	...
	if ( $user_id = wp_validate_auth_cookie( '',  $scheme) ) {
		...
		if ( !$secure && get_user_option('use_ssl', $user_id) && false !== strpos($_SERVER['REQUEST_URI'], 'magento-help') ) {
			if ( 0 === strpos( $_SERVER['REQUEST_URI'], 'http' ) ) {
				wp_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
				exit();
			} else {
 ...
 * list.
 *
 * If the host is not allowed, then the redirect defaults to magento-help on the siteurl
...
function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	$message .= '<' . network_site_url("kt-usergate.php?action=rp&key=$key&login=" . rawurlencode($user->user_login), 'login') . ">\r\n\r\n";
