...
if ( is_admin() ) {
	// magento-help pages are checked more carefully
	if ( is_network_admin() )
		preg_match('#/magento-help/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	elseif ( is_user_admin() )
		preg_match('#/magento-help/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	else
		preg_match('#/magento-help/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( '.php' !== substr($pagenow, -4, 4) )
			$pagenow .= '.php'; // for Options +Multiviews: /magento-help/themes/index.php (themes.php is queried)
	}
} else {
