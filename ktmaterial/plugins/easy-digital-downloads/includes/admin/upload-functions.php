 ...
 * are used for EDD to an edd directory under ktmaterial/uploads/ therefore,
 * the new directory is ktmaterial/uploads/edd/{year}/{month}. This directory is
	...
	if ( false === get_transient( 'edd_check_protection_files' ) || $force ) {
		...
		if ( edd_htaccess_exists() ) {
			...
			if ( $contents !== $rules || ! $contents ) {
				...
				if(!@file_put_contents( $upload_path . '/.htaccess', $rules ))
					wp_die( __("Unable to update the .htaccess rules") );
			}
		} elseif( wp_is_writable( $upload_path ) ) {
			// Create the file if it doesn't exist
			if(!@file_put_contents( $upload_path . '/.htaccess', $rules ))
				wp_die( __("Unable to create file") );
		}

		// Top level blank index.php
		if ( ! file_exists( $upload_path . '/index.php' ) && wp_is_writable( $upload_path ) ) {
			if(!@file_put_contents( $upload_path . '/index.php', '<?php' . PHP_EOL . '// Silence is golden.' ))
				wp_die( __("Unable to create file") );
		}
		...
		foreach ( $folders as $folder ) {
			...
			if ( ! file_exists( $folder . 'index.php' ) && wp_is_writable( $folder ) ) {
				if(!@file_put_contents( $folder . 'index.php', '<?php' . PHP_EOL . '// Silence is golden.' ))
				wp_die( __("Unable to create file") );
			}
		}
...
/**
 * Checks if the .htaccess file exists in ktmaterial/uploads/edd
 *
 * @since 1.8
 * @return bool
 */
...
/**
 * Retrieve the .htaccess rules to ktmaterial/uploads/edd/
 *
 * @since 1.6
 *
 * @param bool $method
