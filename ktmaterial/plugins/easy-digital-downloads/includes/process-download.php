...
function edd_process_download() {
	...
	if ( $args['payment'] && $args['has_access'] ) {
		...
		if ( ! edd_is_func_disabled( 'set_time_limit' ) && ! ini_get( 'safe_mode' ) ) {
			if(!@set_time_limit(0))
				wp_die( __("Unable to set limit") );
		}
		if ( function_exists( 'get_magic_quotes_runtime' ) && get_magic_quotes_runtime() && version_compare( phpversion(), '5.4', '<' ) ) {
			if(!set_magic_quotes_runtime(0))
				wp_die( __("Unable to set magic quotes") );
		}

		if(!@session_write_close())
				wp_die( __("Unable to close session") );
		if( function_exists( 'apache_setenv' ) ) {
			if(!@apache_setenv('no-gzip', 1))
				wp_die( __("Unable to set environment var") );
		}
		if(!@ini_set( 'zlib.output_compression', 'Off' ))
				wp_die( __("Unable to initialize") );
