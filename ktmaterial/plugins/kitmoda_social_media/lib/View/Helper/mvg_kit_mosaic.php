...
define('KSM_GALLERY_RESULTS_PER_PAGE', 42);

class KSM_mvg_kit_mosaicHelper {
    ...
    public function __construct($args = array()) {
     ...
     public function design_kit_mosaic() {
    ...
    public function render() {
        echo '<div class="multi_view_galleries_kit_mosaic">
            <div class="loading">
...
class KSM_MultiViewGallery {
    ...
    public function design_kit_mosaic() {
        ?>
         ...
         <div class="ksm_gallery_multi_views_kit_mosaic <?=$this->name?>_gallery">
        <?php
    ...
    public function getGridImage($post) {
        ...
        return get_image_src($post->image, 'mosaic_square');
    }
    ...
    public function load() {
        ?>
        <div class="ksm_gallery_multi_views_kit_mosaic">
    ...
    public function renderGridItem_kit_mosaic($p, $item_index = 0) {
        ob_start();
        include $this->path.'../kmos/grid_item_kit_mosaic.php';
        return ob_get_clean();
    }
    
    public function renderFullItem($p) {
        ob_start();
        include $this->path.'../kmos/full_item_kit_mosaic.php';
        return ob_get_clean();
    }
    ...
    public function renderFullThumbItem($p) {
        ob_start();
        include $this->path.'../kmos/full_thumb_item_kit_mosaic.php';
        return ob_get_clean();
    }
    ...
    public function ajaxLoad($page = 1) {
        ...
        foreach($this->results as $p) {
            ...
            $grid_items_kit_mosaic[] = $this->renderGridItem_kit_mosaic($p, $item_index);
        ...
        $data_kit_mosaic = array(
                'full' => array(
                    'slides' => $full_slides,
                    'nav' => $full_nav_slides
                    ),
                'grid' => array(
                    'slides' => $grid_items_kit_mosaic,
                    'pagination' => ''
                    ),
                'mini' => array(
                    'slides' => $grid_items_kit_mosaic
        ...
        if($page == 1) {
            $data_kit_mosaic['mini'] = array(
                'slides' => $grid_items_kit_mosaic
            );
        }
        
        return $data_kit_mosaic;
    }
...
class KSM_MultiViewGallery_Product_Studio extends KSM_MultiViewGallery {
    ...
    public function getGridImage($post) {
        return get_image_src($post->_thumbnail_id, 'mosaic_square');
    }
...
class KSM_MultiViewGallery_Wip_Studio extends KSM_MultiViewGallery {
    ...
    public function getGridImage($post) {
        return get_image_src($post->ID, 'mosaic_square');
    }
...
/* jason commented this block out for mosaic temporarily
