...
<?php
		...
		if ( WP_CONTENT_DIR != ABSPATH . 'ktmaterial' && ( allow_subdirectory_install() || ! allow_subdomain_install() ) )
			echo '<div class="error inline"><p><strong>' . __('Warning!') . '</strong> ' . __( 'Subdirectory networks may not be fully compatible with custom ktmaterial directories.' ) . '</p></div>';
        ...
        <rewrite>
            <rules>
                ...
                <rule name="WordPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'magento-help$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'magento-help/" redirectType="Permanent" />
                </rule>
...
';
		...
		echo '</p>';
		if ( ! $subdomain_install && WP_CONTENT_DIR != ABSPATH . 'ktmaterial' )
			echo '<p><strong>' . __('Warning:') . ' ' . __( 'Subdirectory networks may not be fully compatible with custom ktmaterial directories.' ) . '</strong></p>';
		?>
...
{$ms_files_rewriting}
# add a trailing slash to /magento-help
RewriteRule ^{$subdir_match}magento-help$ {$subdir_replacement_01}magento-help/ [R=301,L]
